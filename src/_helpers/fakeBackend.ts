...
let users = JSON.parse(localStorage.getItem(process.env.VUE_APP_LOCAL_STORAGE_KEY) as string) || [];    
export function configureFakeBackend() {

    let realFetch = window.fetch;
    window.fetch = function (url: any, opts: any): Promise<any> {
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                ...
                if (url.endsWith('/users/register') && opts.method === 'POST') {
                    ...
                    users.push(newUser);
                    localStorage.setItem(process.env.VUE_APP_LOCAL_STORAGE_KEY, JSON.stringify(users));
